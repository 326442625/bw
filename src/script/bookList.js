...
require(["entry"], function (CONST) {
    require(["common", "date"], function ($) {
        $(function () {
            ...
            function getAllData(filterParam, timeParam) {
                var param = {};
                var timeUrl = '';
                ...
                if (type == 1) { // 普通搜索
                    ...
                    $.getData({
                        ...
                        success: function (data) {
                            ...
                            if (oReturn) {
                                $.loadMore(data.data, 'book_list?type=search&' + param + '&' + filterParam, callBack);
                            }
                        }
                    })
                } else if (type == 2) { // 高级搜索
                    ...
                    $.getData({
                        ...
                        success: function (data) {
                            var oReturn = callBack(data);
                            if (oReturn) {
                                $.loadMore(data.data, 'book_list?type=search&' + urlParam + '&' + filterParam + timeUrl, callBack);
                            }
                        }
                    })
                    ...
                    $.getData({
                        ...
                        success: function (data) {
                            var oReturn = callBack(data);
                            if (oReturn) {
                                $.loadMore(data.data, reqUrl, callBack);
                            }
                        }
                    })
                } else if (type == 4) { //新书
